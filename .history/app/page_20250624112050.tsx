  ...
  return (
    <div className="w-80 p-6 bg-white/90 backdrop-blur rounded-2xl shadow-2xl border border-white/20">
      ...
      <div className="relative mb-6">
        ...
        {/* sliding controller */}
        <motion.div
          className="absolute top-1/2 w-6 h-6 -mt-3 -ml-3 bg-white rounded-full shadow-lg border-2 border-slate-300 cursor-pointer z-20"
          style={{ left: `${sliderValue}%` }}
          whileHover={{ scale: 1.2 }}
          layout
          transition={{ duration: 0.2 }}
          onMouseDown={(e) => {
          // 点击 controller 相当于点击 input → 派发事件
          const input = e.currentTarget.parentElement?.querySelector("input[type=range]") as HTMLInputElement;
          input?.focus(); // 确保焦点
          }}
        >
